...
Y1 = Y(1:1:N2, 1:1:M2);
for x = 2:vertical
    for z = 1:(horizontal-1)
    Y1 = Y((N2*(x - 1) + 6):1:((N2*x)+5), (M2*(z - 1) + 1):1:(M2*z));
      for k = 45:45:360
         %TMP = imrotate(X, k, 'bilinear', 'crop'); 
         %corr = corr2(double(Y1), double(TMP));
         TMP = imrotate(Y1, k, 'bilinear', 'crop'); 
...
hold on
for i = 1:(d-1)
    plot(ceil((2*match_dims{i}(1, 1)+match_dims{i}(1,3))/2), ceil((match_dims{i}(1,2)+match_dims{i}(1,4)/2)), 'r.', 'MarkerSize',20);
